...
// ════════════════════════════════════════════════════════════
// SCROLL DISSOLVE REVEAL — THREE.JS SHADER CONTROLLER
// ════════════════════════════════════════════════════════════
(function () {
  const container = document.getElementById('dissolve-container');
  ...
  // UI elements
  const cardEl = document.getElementById('dissolve-card');
  const dPill = document.getElementById('d-pill');
  const dPlatform = document.getElementById('d-platform');
  const dTitle = document.getElementById('d-title');
  const dHandle = document.getElementById('d-handle');
  const dScope = document.getElementById('d-scope');
  const dDesc = document.getElementById('d-desc');
  const dChips = document.getElementById('d-chips');
  const dStat1 = document.getElementById('d-stat1');
  const dStat1Lbl = document.getElementById('d-stat1-lbl');
  const dStat2 = document.getElementById('d-stat2');
  const dStat2Lbl = document.getElementById('d-stat2-lbl');
  const dCta = document.getElementById('d-cta');
  const dCtaText = document.getElementById('d-cta-text');
  const scrubSteps = document.querySelectorAll('.dissolve-scrub-step');
  ...
  function updateCard(idx) {
    ...
    setTimeout(() => {
      if (dPill) dPill.textContent = data.pill;
      if (dPlatform) dPlatform.textContent = data.platform;
      if (dTitle) dTitle.textContent = data.title;
      if (dHandle) dHandle.textContent = data.handle;
      if (dScope) dScope.innerHTML = `<span>${data.scope}</span>`;
      if (dDesc) dDesc.textContent = data.desc;
      if (dStat1) dStat1.textContent = data.stat1;
      if (dStat1Lbl) dStat1Lbl.textContent = data.stat1Lbl;
      if (dStat2) dStat2.textContent = data.stat2;
      if (dStat2Lbl) dStat2Lbl.textContent = data.stat2Lbl;
      if (dCtaText) dCtaText.textContent = data.ctaText;
      if (dCta) dCta.href = data.ctaLink;

      if (dChips) {
        dChips.innerHTML = data.chips.map(c => `<span class="pc-chip">${c}</span>`).join('');
      }

      scrubSteps.forEach((step, i) => {
  ...
  // ── GLSL SHADERS (Matching user's exact specification) ──
  ...
  // Click scrub step navigation
  scrubSteps.forEach((step, i) => {
    step.addEventListener('click', () => {
      const rect = container.getBoundingClientRect();
      const totalScrollable = container.offsetHeight - window.innerHeight;
      const targetScroll = window.scrollY + rect.top + (i / 2) * totalScrollable;
  ...
  function animate() {
    ...
    // Material 1 (Front Dissolve Mesh)
